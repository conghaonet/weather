...
class PermissionUtil {
  static Future<PermissionGroup> requestPermissions(BuildContext context, List<PermissionGroup> permissions, String promptMsg, {bool showPrompt = true, SnackBarAction action}) async {
    ...
    if(showPrompt && deniedPermission != null) {
      ...
      if(!isShown) {
        showPermissionPrompt(context, deniedPermission, promptMsg: promptMsg, action: action);
      }
    }

    return deniedPermission;
  }
  static showPermissionPrompt(BuildContext context, PermissionGroup permissionGroup, {String promptMsg, SnackBarAction action}) async {
    bool isShown = await PermissionHandler().shouldShowRequestPermissionRationale(permissionGroup);
    if(!isShown) {
      Util.showSnackBar(context, strContent: promptMsg, action: action);
    }
  }
